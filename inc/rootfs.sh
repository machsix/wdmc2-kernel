...
EOF
    ...
    [[ "$BOOT_DEVICE" == 'usb' ]] && chroot "${rootfs_dir}" /bin/bash -c "ln -sfr /etc/fstab.usb /etc/fstab"
    [[ "$BOOT_DEVICE" == 'hdd' ]] && chroot "${rootfs_dir}" /bin/bash -c "ln -sfr /etc/fstab.hdd /etc/fstab"
